...
customIndex_fix={'A': 0.006561418753371375, 'C': 4.7271214720367993e-17, 'D': 0.05793278267445551, 'E': 0.07710008561159147, \
 'F': -0.013688206829502714, 'G': 0.0322978200732179, 'H': -0.2579153742971864, 'I': -0.022810656606072642, \
 'K': -0.17075414630022018, 'L': 0.04425042739949629, 'M': 0.05525787919957905, 'N': 0.007309839349683702, \
 'P': 0.07124562426548042, 'Q': 0.0072292086728551214, 'R': -0.29572839441059284, 'S': 0.025211449406263867, \
 'T': 0.04027345660879873, 'V': 0.028848517504872974, 'W': 0.18564457320864294, 'Y': -0.01045574370488163}
...
def buildRetentionIndex(aaAlphabet, retentionTimes,seqs,
      normalizeRetentionTimes):
  featureMatrix = computeRetentionIndexFeatureMatrix(aaAlphabet, seqs)
  #print(featureMatrix)
...
def computeRetentionIndexFeatureMatrix(aaAlphabet, seqs):
  featureMatrix = np.zeros((len(seqs), len(aaAlphabet)))
...
#  else:
  for i, seq in enumerate(seqs):
      featureMatrix[i] = computeRetentionIndexFeatures(aaAlphabet, seq)
...
def computeRetentionFeatureVector(aaAlphabet, peptide, customIndex):
  ...
  #customIndex=kyteDoolittleIndex
  #customIndex=customIndex_fix
